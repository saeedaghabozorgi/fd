...
#             # https://www.tensorflow.org/performance/performance_guide#data_formats
#             inputs = tf.transpose(inputs, [0, 3, 1, 2])
        #inputs= tf.layers.dense(inputs=inputs, units=labels_size)
        inputs = tf.layers.dense(inputs, 10, activation=tf.nn.elu,name = "hidden{}".format(1))
        inputs = tf.nn.elu(inputs)
        inputs = tf.layers.dense(inputs=inputs, units=2)
        inputs = tf.identity(inputs, 'final_dense')
        return inputs
